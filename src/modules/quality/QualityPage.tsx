...
import { ncrs as initialNCRs, capas as initialCAPAs } from '@/json/quality_ncrs'
...
const ncrColumns = [
  {
    ...
    header: 'NCR #',
    sortable: true,
    render: (r: NCR) => <span className="font-mono text-xs text-red-400">{r.ncrNumber}</span>,
  },
  {
    key: 'severity',
    header: 'Severity',
    ...
    render: (r: NCR) => (
      <span
        className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${severityColors[r.severity]}`}
      >
  ...
  {
    key: 'clause',
    header: 'AS9100D Clause',
    sortable: true,
    render: (r: NCR) => (
      <span className="text-xs text-text-muted font-mono">{r.clause.split(' —')[0]}</span>
    ),
  },
  {
    key: 'raisedBy',
    header: 'Raised By',
    sortable: true,
    render: (r: NCR) => <span className="text-xs text-text-muted">{r.raisedBy}</span>,
  },
  {
    key: 'status',
    header: 'Status',
...
const capaColumns = [
  {
    ...
    header: 'CAPA #',
    sortable: true,
    render: (r: CAPA) => <span className="font-mono text-xs text-amber-ops">{r.capaNumber}</span>,
  },
  {
    key: 'rootCause',
    header: 'Root Cause',
    sortable: true,
    render: (r: CAPA) => (
      <span className="text-xs text-text-muted max-w-[200px] truncate block">{r.rootCause}</span>
    ),
  },
  {
    key: 'owner',
    header: 'Owner',
    sortable: true,
    render: (r: CAPA) => <span className="text-xs text-text-muted">{r.owner}</span>,
  },
  {
    key: 'status',
    header: 'Status',
  ...
  },
  {
    ...
    header: 'Target',
    sortable: true,
    render: (r: CAPA) => <span className="text-xs font-mono text-text-muted">{r.targetDate}</span>,
  },
]

const QualityPage: React.FC = () => {
  const { ncrs, capas, updateNCR, setNCRs, setCAPAs } = useQualityStore()
  ...
  const handleMRBReview = () => {
    if (!selectedNCR) return
    updateNCR(selectedNCR.id, { status: 'under_review' })
    setSelectedNCR({ ...selectedNCR, status: 'under_review' })
  }

  const handleCloseNCR = () => {
    if (!selectedNCR) return
    const closedAt = new Date().toISOString()
    updateNCR(selectedNCR.id, { status: 'closed', closedAt })
    setSelectedNCR({ ...selectedNCR, status: 'closed', closedAt })
  }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-text-main">
          AS9100D Quality & Non-Conformance Center
        </h1>
        <p className="text-xs text-text-muted mt-1">
          Defense & Aerospace Quality Management System — NCR / MRB / CAPA
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPICard
          ...
          value={closedCAPAs}
          unit={`/ ${capas.length}`}
          status={closedCAPAs / capas.length > 0.5 ? 'pass' : 'warning'}
        />
      </div>
        ...
        {selectedNCR && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <AlertTriangle
                className={`w-5 h-5 ${severityColors[selectedNCR.severity].split(' ')[0]}`}
              />
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div className="bg-slate-800/50 rounded-lg p-3">
                ...
                <p className="font-mono text-text-main">
                  {new Date(selectedNCR.raisedAt).toLocaleDateString()}
                </p>
              </div>
              <div className="bg-slate-800/50 rounded-lg p-3">
            ...
            <div className="flex gap-2 pt-2">
              <button
                onClick={handleMRBReview}
                disabled={selectedNCR.status !== 'open'}
                className="flex-1 px-3 py-2 bg-blue-500/10 border border-blue-500/30 rounded-lg text-xs font-medium text-blue-400 hover:bg-blue-500/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Assign to MRB Review
              </button>
              <button
                onClick={handleCloseNCR}
                disabled={selectedNCR.status === 'closed'}
      ...
      <div className="card-gradient p-4 space-y-3">
        ...
        <EnterpriseDataTable data={capas.slice(0, 20)} columns={capaColumns} pageSize={8} />
      </div>
    </div>
  )
}
