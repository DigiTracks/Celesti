...
const AnalyticsPage: React.FC = () => {
  ...
  const fpyData = facilities.map((f) => ({
    facility: f.name.includes('Hyderabad')
      ? 'Hyderabad'
      : f.name.includes('Bengaluru')
        ? 'Bengaluru'
        : 'Thiruvananthapuram',
    fpy: f.fpy,
    oee: f.oee,
    orders: f.activeOrders,
  }))
    ...
    <div className="space-y-6" data-tour="spc-chart">
      <div>
        ...
        <p className="text-xs text-text-muted mt-1">
          Statistical Process Control — Yield, OEE, Defect Trend Analysis
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPICard
          ...
          unit="%"
          status={avgFPY > 93 ? 'pass' : 'warning'}
          trend={{ value: 2.3, isPositive: true }}
        />
        <KPICard
          title="Avg Overall OEE"
          value={avgOEE.toFixed(1)}
          unit="%"
          status={avgOEE > 85 ? 'pass' : 'warning'}
        ...
        <KPICard
          ...
          value={onTimePct}
          unit="%"
          status={onTimePct > 85 ? 'pass' : 'warning'}
        />
      </div>
              ...
              <div key={d.facility} className="space-y-1.5">
                ...
                <div className="h-2 bg-slate-800 rounded-full overflow-hidden flex">
                  ...
                  <div
                    ...
                    style={{ width: `${Math.max(0, d.oee - d.fpy)}%` }}
                  />
                </div>
                <div className="flex items-center gap-3 text-[10px] text-text-muted">
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-emerald-pass" /> FPY
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-blue-400" /> OEE
                  </span>
      ...
      <div className="card-gradient p-4 space-y-3">
        ...
        <div className="grid grid-cols-6 gap-2">
          {['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((month, i) => {
            const yieldVals = [93.2, 94.8, 92.1, 95.3, 94.0, 96.5]
            const yieldVal = yieldVals[i]!
            return (
